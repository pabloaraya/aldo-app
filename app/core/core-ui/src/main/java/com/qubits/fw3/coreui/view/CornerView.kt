...
class CornerView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val cornerRadiusIn = convertDpToPixel(context, resources.getDimension(R.dimen.corner_radius_24dp))
...
//        )
        ...
        canvas.drawPath(
            createCornersPath(
                top = 8.toFloat(), //top,
                left = 8.toFloat(), //oneSix,//(width*0.075).toFloat(),
                right = (width - 8).toFloat(), //(width.toFloat() - oneSix),//(width*0.925).toFloat(),
                bottom = (height - 8).toFloat(), //bottom,
                cornerRadius = cornerRadius,
                cornerLength = (width/6).toFloat(),//cornerRadiusLength
            ), cornerPaint
        )
    }
